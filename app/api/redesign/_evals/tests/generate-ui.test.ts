...
test("validateRenderability: valid inline HTML passes", () => {
  ...
  assert.doesNotThrow(() => validateRenderability(html));
});

test("validateRenderability: <script> tag throws", () => {
  assert.throws(
    () => validateRenderability(`<html><body><script>x</script></body></html>`),
    /<script>/,
  );
});
...
test("validateRenderability: <script type=...> throws", () => {
  assert.throws(
    () => validateRenderability(`<script type="text/javascript">x</script>`),
    /<script>/,
  );
});

test("validateRenderability: uppercase <SCRIPT> throws (case-insensitive)", () => {
  assert.throws(() => validateRenderability(`<SCRIPT>x</SCRIPT>`), /<script>/);
});
...
test("validateRenderability: <link> with data-URI href passes", () => {
  assert.doesNotThrow(() =>
    validateRenderability(`<link rel="icon" href="data:image/png;base64,x">`),
  );
});

test("validateRenderability: external image URL throws", () => {
  assert.throws(
    () => validateRenderability(`<img src="https://example.com/x.png">`),
    /external image/,
  );
});
...
test("validateRenderability: data-URI image passes", () => {
  assert.doesNotThrow(() =>
    validateRenderability(`<img src="data:image/png;base64,iVBORw0KGgo=">`),
  );
});
...
test("validateRenderability: HTML over 50KB throws", () => {
  ...
  assert.throws(() => validateRenderability(html), /size ceiling/);
});
